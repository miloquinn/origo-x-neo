...
当前共享顶栏在 412×84dp、手机旧后端降级参数 16 段下，用本机 Metal `flutter_tester` 做 5 次预热、50 次离屏渲染及 RGBA 回读，每种配置顺序交替运行三批。DPR 2 开启模糊的批次中位数为 2.472/1.770/2.431ms，关闭为 1.131/1.049/1.039ms；DPR 3 开启为 3.501/3.566/3.887ms，关闭为 1.426/1.282/1.314ms。顶边镜像修复只在顶部越界的纵向采样改变坐标，没有增加纹理读取次数。这些值包含回读与调度，不是屏幕帧耗时或实体 iPhone FPS；仍需真机 profile 才能确认滚动帧率。

旧的 `progressive_blur.dart` 和未被生产代码调用的预设、帮助 API 已删除。阅读控制栏直接使用统一配置；顶部组件收到 `blurEnabled: false` 时不初始化 Shader，重新开启时按需加载。
