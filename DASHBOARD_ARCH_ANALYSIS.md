...
- MemberAccountController (member_account_controller.dart, 682 lines): ChangeNotifier exposing authConfig, membershipConfig, user/membership/summary/referral/mfaStatus, loading/error; wires collaborators (MemberAccountApiClient, AccountAvatarCache, MemberAccountSummaryCache, SecurePendingDeviceAuthorizationStore, AccountAuthCallbackBridge, ApplePremiumPurchaseService).
  ...
  - Store purchase (updated 2026-09-26): StorePurchaseService separates guest reader/trial, account Premium and legacy bundle verification. Reader purchase/restore uses an installation credential; Premium requires permanent reader ownership and the signed-in Origo account.
...
- Store purchase (store_purchase_service.dart): PurchaseStore + InAppPurchaseStore share one purchaseStream across Google and Apple, with separate reader/Premium states. Verification failure keeps transactions retryable; restores wait for relevant receipt verification. Apple native sync filters requested product IDs. See docs/plans/2026-09-26-independent-store-products.md for current prices, trial and migration rules.
