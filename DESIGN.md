...
## 基础版与高级会员购买页面（2026-09-27）

- 用户要求：购买页不能把权益、价格、恢复、协议堆成网页式长页；名称由“应用解锁”改为“基础版购买”。本节取代先前购买页面的长页布局及相关显示术语，SKU和权益含义不变。
- 观察：现有两个页面用ListView依次铺开大卡片、试用说明、价格、购买、恢复说明、完整权益与条款，主操作会随滚动离屏。已检查正式Flutter基础版截图和手机高级版截图。
- 调整顺序：先保留购买/恢复现有回归；提取两页共用的轻量购买布局；基础版与高级会员分别压缩摘要；完整说明放原生二级页；统一多语文案；检查正式Flutter渲染与交互回归。
- 首屏：品牌/会员身份摘要、简短分类权益、当前账户或试用状态、详情入口。基础版用两列功能概览突出完整阅读体验，高级会员显示两项进阶扩展。底部集中真实商店价格、一次购买说明、主操作；基础版同时保留试用和恢复。
- 二级页：完整权益；购买及恢复说明；会员条款、隐私及Apple EULA。所有入口有明确名称和返回行为，详情不触发购买，退出详情保留实时交易状态。
- 视觉：复用现有主题、FloatingSubpageScaffold、品牌图标、Premium卡片色彩。内容最大480px，24px外边距，52px主按钮；不用新依赖和第二套主题。
- 响应：正常390×844手机摘要与操作无需长滚动，底部操作稳定。低于620px可用高度、键盘或字体超过1.35倍时回退为整体可滚动内容，避免截断购买/兑换与错误信息。二级说明页允许正常滚动。
- 状态：保留访客购买基础版、试用、已购买、商店未就绪/重试、pending、恢复、取消、失败和旧高级会员来源；账号变化仍实时更新。价格必须来自商店商品，不硬编码；商店不出现卡密，官网仍提供高级版兑换。
- 验证：两页分别独立进程测试，覆盖正常手机首屏可操作、320px/大字体/键盘、原生详情来回与权益门槛；正式Flutter浅深色预览并人工检视。无visual-verdict技能文件可用，使用人工视觉审查并保存同等JSON记录。

- 用户追加定位：基础版包含完整阅读工具，按阅读、主题字体、朗读、云端 TTS、AI、书库备份分类展示，完整能力进入权益二级页。高级版表述为“进阶扩展”；避免“全部功能”和“隐藏功能”混淆独立购买边界。AI/云端 TTS 自带服务配置及第三方费用说明。

- 实施证据：`docs/purchase-ui-redesign.md`、`docs/previews/purchase-redesign/`；52项界面/权益回归通过，静态分析无问题。预览使用正式组件和演示账号，未发布新商店构建。


## 购买页视觉方向二（2026-09-27，待用户评审）

- 用户否定前版功能图标列表的质感，要求免费开源素材支持更完整的能力展示。此次先制作可交互视觉提案，不将未接受的设计写入 Flutter。
- 提案位于 `docs/previews/purchase-editorial/index.html`：基础版用公版画作书封、原创建模纸页与编辑式排版，分阅读排版、听书 AI、记录备份三组横向切换；全部八组功能进入二级权益页。高级版采用同一家族的深绿色与低饱和金色，以书源连接表达进阶扩展。
- 固定底部价格/购买/恢复/试用区域，保留无账号基础版与账号绑定高级版的区别。预览价格和账号均为示意，不触发交易。
- 素材使用 Phosphor MIT SVG 与 The Met Open Access 的两幅公版 CC0 画作；文件、SHA256、许可、出处见同目录 `ASSETS.md` 与 `assets/manifest.json`。不新增依赖，不把在线素材请求放入生产 App。
- 实际浏览器检查：三个能力场景、权益二级页与返回、窄屏、素材加载及控制台。完整 Flutter、多语言、大字模式和真机验收属于方向接受后的生产实现步骤。本提案未发布到商店。


### 方向二已获批准，进入 Flutter（2026-09-27）

用户回复“还不错，那就这样吧”，采用方向二。实施顺序：保留已通过的权益回归；复用购买布局加局部主题；本地打包公版画作并保留许可；原生 Stack/Transform 组成书封、纸页、听书与书源视觉；Basic 三组选择控件与全部权益二级页；Premium 进阶展示；十语文案、实际 Flutter 渲染及小屏/大字/状态回归。装饰画面不参与屏幕阅读器语义、不跟随辅助字号放大，交互文字正常遵循字号设置。价格仍来自商店，图片不走运行时网络请求。

- 方向二生产实现完成：正式 Flutter 场景见 `docs/previews/purchase-editorial/implemented/index.html`；原生分类选择、局部纸张/深林主题、许可页、三组装饰视觉与书源网络构图均已接入。36项界面/许可回归通过；保留小屏、键盘、放大文字、德语与旧权益分支。未上传或安装新商店构建。
